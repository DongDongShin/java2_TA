...
//		Stream<String> words = Stream.of(sa);
        words.forEach(s -> System.out.print(s + " "));//B C
        System.out.println();

        IntStream ints = IntStream.range(10, 20);
        ints.forEach(s -> System.out.print(s + " "));//10~19
        System.out.println();

        Stream<String> seasons = Stream.of("하지", "동지", "춘분", "추분", "입동");
        seasons.forEach(s -> System.out.print(s + " "));//랜덤순으로 출력?
        System.out.println();

        // 빈 스트림
        Stream<String> silence = Stream.empty();
        silence.forEach(s -> System.out.print(s + " "));//빈칸
        System.out.println();

        // 필요할 때마다 "야옹"을 만든다. 무한히 긴 스트림이다.
        Stream<String> yaongs = Stream.generate(() -> "야옹");
        yaongs.limit(10).forEach(s -> System.out.print(s + " "));//야옹 야옹 ...
        System.out.println();

        // 필요할 때마다 double 난수를 만든다. 무한히 긴 스트림이다.
        Stream<Double> randoms = Stream.generate(Math::random);
        randoms.limit(4).forEach(s -> System.out.print(s + " "));//random random random
        System.out.println();
        ...
        integers.limit(4).forEach(s -> System.out.print(s + " "));//0 1 2 3
        System.out.println();
        ...
        bIntegers.limit(4).forEach(s -> System.out.print(s + " "));//0 1 2 3
        System.out.println();
